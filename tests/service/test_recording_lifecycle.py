...
def _make_lifecycle(config_dict: dict | None = None):
    ...
    return lifecycle, master, recorder, audio_file_manager, transcription_handler, firestore_output, ui_processor
...
class TestRecordingLifecycleInit:
    ...
    def test_init_success(self):
        """正常系: 正常初期化"""
        lifecycle, master, recorder, afm, th, fs, ui = _make_lifecycle()
        ...
        assert lifecycle.firestore_output == fs
...
class TestRecordingLifecycleSafeUiUpdate:
    ...
    def test_safe_ui_update_calls_firestore_append(self):
        """正常系: FirestoreOutput.appendを呼ぶ"""
        lifecycle, _, _, _, _, fs, ui = _make_lifecycle()
        ui.is_ui_valid.return_value = True

        lifecycle._safe_ui_update("テキスト")

        fs.append.assert_called_once_with("テキスト")
